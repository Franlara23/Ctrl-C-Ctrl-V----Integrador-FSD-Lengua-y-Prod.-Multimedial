...
function cargarCartasSeleccionadas() {
  ...
  contenedores.forEach(contenedor => {
    ...
    const key = `cartas_seleccionadas_j2_${recetaNum}`;
...
function hacerEditablesRecetas() {
  ...
  botones.forEach(btn => {
    ...
    const nombreGuardado = localStorage.getItem(`receta_nombre_j2_${recetaNum}`);
    if (nombreGuardado) {
    ...
    btn.addEventListener('blur', function() {
      ...
      localStorage.setItem(`receta_nombre_j2_${recetaNum}`, nombreNuevo);
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (t) {
    const nombreGuardado = localStorage.getItem('nombreJugador2');
    t.textContent = nombreGuardado || 'Jugador 2';
  }
